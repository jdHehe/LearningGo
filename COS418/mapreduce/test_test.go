...
// 配置 master和worker的地址，原文 采用unix 文件通信方式，但是此法在windows上行不通
func port(suffix string) string {
	//s := "D://learningMaterial//GoLanguage//src//github.com//jdHeHe//LearningGo//COS418//mapreduce//824-"
	//s += strconv.Itoa(os.Getuid()) + "/"
	//os.Mkdir(s, 0777)
	//s += "mr"
	//s += strconv.Itoa(os.Getpid()) + "-"
	//s += suffix
	if strings.Contains(suffix, "master") {
		return "localhost:3000"
	}else if strings.Contains(suffix, "worker") {
		count_string := strings.TrimLeft(suffix, "worker")
		count_number, err := strconv.Atoi(count_string)
		if err != nil{
			panic(err)
		}
		port := 4000+count_number
		port_string := strconv.Itoa(port)
		return "localhost:"+port_string
	}else {
		panic("isn't worker or matser")
	}
}

func setup() *Master {
	files := makeInputs(nMap)
	//master := port("master.sock")
